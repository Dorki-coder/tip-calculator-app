  ...
  return (
    <>
      ...
      <div className="card">
        <div className="card__input">
          ...
          <label className="input__Bill">
            <input
              ...
              placeholder="0"
              value={billAmount}
              onChange={(el) => setBill(el.target.value)}
            ></input>
          </label>
          <h2>Select tip %</h2>
          <div className="select__tip">
            <button
              className={tipCount === 1.05 ? "active" : null}
              onClick={() =>
                tipCount === 1.05 ? setTipCount(1) : setTipCount(1.05)
              }
            >
              5%
            </button>
            <button
              className={tipCount === 1.1 ? "active" : null}
              onClick={() =>
                tipCount === 1.1 ? setTipCount(1) : setTipCount(1.1)
              }
            >
              10%
            </button>
            <button
              className={tipCount === 1.15 ? "active" : null}
              onClick={() =>
                tipCount === 1.15 ? setTipCount(1) : setTipCount(1.15)
              }
            >
              15%
            </button>
            <button
              className={tipCount === 1.25 ? "active" : null}
              onClick={() =>
                tipCount === 1.25 ? setTipCount(1) : setTipCount(1.25)
              }
            >
              25%
            </button>
            <button
              className={tipCount === 1.5 ? "active" : null}
              onClick={() =>
                tipCount === 1.5 ? setTipCount(1) : setTipCount(1.5)
              }
            >
              50%
            </button>
            <input
              ...
              placeholder="Custom"
              onChange={(e) =>
                billAmount > 0 && peopleCount > 0
                  ? setTipCount(1 + e.target.value / 100)
                  : null
              }
            ></input>
          </div>
          <h2>Number of People</h2>
          <label className="input__numberOfPeople">
            <input
              type="number"
              value={peopleCount}
              placeholder="0"
              min="1"
              onChange={(el) => setPeopleCount(el.target.value)}
            ></input>
          </label>
        </div>
        <div className="card__output">
          <div className="card__outline">
            <div>
              <div className="card__outline_title "> Tip amount</div>
              <div className="card_outline_subtitle">/person</div>
            </div>
            <span className="price">
              $
              {isNaN(
                Math.round(
                  ((billAmount * tipCount - billAmount) / peopleCount) * 100
                ) / 100
              )
                ? 0
                : Math.round(
          ...
          <div className="card__outline">
            <div>
              <div className="card__outline_title">Total</div>
              <div className="card_outline_subtitle"> / person</div>
            </div>
            <span className="price">
              $
              {isNaN(
                Math.round(((billAmount * tipCount) / peopleCount) * 100) / 100
              )
                ? 0
                : Math.round(((billAmount * tipCount) / peopleCount) * 100) /
                  100}
            </span>
          </div>
      ...
      <div className="attribution">
        ...
        . Coded by <a href="https://github.com/dorki-coder">Dorki</a>.
      </div>
    </>
  );
}
